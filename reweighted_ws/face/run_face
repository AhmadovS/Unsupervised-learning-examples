...
def run_experiment(args):
    ...
    from learning.dataset import FACE
    ...
    if args.rebinarize:
        ...
        dataset = FACE(which_set='train', preproc=binarize_preproc)
        valiset = FACE(which_set='valid', preproc=binarize_preproc)
        testset = FACE(which_set='test', preproc=preproc)
        tags += ["rb"]
    else:
        dataset = FACE(which_set='train', preproc=preproc)
        valiset = FACE(which_set='valid', preproc=preproc)
        testset = FACE(which_set='test', preproc=preproc)
    ...
    trainer = Trainer(
        ...
        dataset=dataset, 
        model=model,
        termination=EarlyStopping(lookahead=args.lookahead, min_epochs=1, max_epochs=5),
        epoch_monitors=[
            DLogModelParams(),
        ],
        final_monitors=[
            ...
            MonitorLL(name="final-trainset", data=dataset, n_samples=n_samples_final),
        ],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rebinarize', default=False, action="store_true", 
        help="Resample binary FACE from orig. dataset during traaining");
